---
title: "Buyback Mechanism"
description: "How the 15-20% creator fees buyback creates sustainable value"
icon: "fire"
---

# Buyback Mechanism

The ENTRO protocol automatically allocates 15-20% of all platform revenue to buy back tokens from the market, creating consistent demand and supporting long-term value.

## Revenue Architecture

### Protocol Revenue Streams

The buyback mechanism is funded through multiple protocol revenue sources:

**Transaction Processing**  
Fees generated from payment processing and settlement operations (0.5% of volume)

**Network Services**  
Revenue from API access, premium features, and institutional services

**Card Operations**  
Card creation, maintenance, and usage fees

### Allocation Framework

**15-20% of all protocol revenue is automatically allocated to buyback operations**, creating consistent market demand for ENTRO tokens through systematic purchases.

## Execution Strategy

### Automated Buyback System

The buyback system operates automatically through smart contracts, ensuring transparency and consistency:

**Execution Process:**
1. Revenue is collected from all platform operations
2. 15-20% is automatically allocated to the buyback wallet
3. Smart contracts execute market purchases regularly
4. All transactions are publicly verifiable on-chain

### Buyback Protocol

- **Allocation Rate**: 15-20% of all protocol revenue
- **Execution Timing**: Regular automated purchases to maintain steady demand
- **Transparency**: All buyback transactions visible on-chain
- **Distribution**: Purchased tokens support staking rewards and ecosystem growth

## Token Allocation

Tokens purchased through the buyback are allocated strategically:

### Staking Rewards Pool (60%)
- Distributed to token stakers over time
- Incentivizes long-term holding
- Enhances staking APY yields

### Liquidity & Ecosystem (40%)
- Liquidity provision for market depth
- Development and operations funding
- Strategic partnerships and growth initiatives

## Impact Analysis

### Supply Dynamics

The 15-20% creator fees buyback creates consistent market demand:
- **Regular buying pressure**: Continuous market purchases
- **Revenue correlation**: Buyback grows with platform success
- **Transparent execution**: All transactions publicly verifiable

### Market Mechanics

The buyback mechanism provides structural support through:
- **Consistent demand**: 15-20% of revenue creates ongoing buying pressure
- **Growth alignment**: More users = more revenue = larger buybacks
- **Value accrual**: Token holders benefit from platform growth

## Transparency

### Public Dashboard

Real-time metrics available at dashboard.entrofinance.com:
- Total protocol revenue generated
- Buyback allocation (15-20% of revenue)
- Tokens purchased through buybacks
- Historical buyback data and trends

### On-Chain Verification

All buyback transactions are verifiable on-chain:
- Buyback wallet address: `BuyBk1111111111111111111111111111111111111`
- Creator fees allocation: 15-20% of all protocol revenue
- Complete transaction history available publicly

## Impact & Growth

### Revenue Scaling

As the platform grows, buyback impact increases:

**Example Projections:**
- $1M monthly revenue → $150-200K in monthly buybacks
- $10M monthly revenue → $1.5-2M in monthly buybacks
- $100M monthly revenue → $15-20M in monthly buybacks

### Long-Term Value

The 15-20% creator fees buyback creates sustainable value:
- Automatic value accrual for token holders
- Revenue-aligned growth mechanism
- Transparent and predictable system
- Benefits all ecosystem participants
